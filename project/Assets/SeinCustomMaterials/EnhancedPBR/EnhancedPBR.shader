...
Shader "Sein/EnhancedPBR" {
    Properties {
        ...
        _refractionOffsetX("Refraction Offset X", Range(-0.1, 0.1)) = 0.03
        _refractionOffsetY("Refraction Offset Y", Range(-0.1, 0.1)) = 0.03
    ...
    CGINCLUDE
        ...
        int _refractionPass;
        float _refractionOffsetX;
        float _refractionOffsetY;
        ...
        float4 gamma(float4 color, v2f i) {
            if (unlit) {
                return color;
            }
     ...
     SubShader {
        ...
        Pass
        {
            ...
            float4 processLight(fixed4 baseColor, v2f i) {
                ...
                if (_refractionPass == 1) {
                    float2 bump = pbr.N.xy;
                    float2 screenUV = i.screenPos.xy / i.screenPos.w - bump * float2(-_refractionOffsetX, _refractionOffsetY);
                    float4 screenColor = tex2D(_refractionMap, screenUV);

                    if (baseColor.a <= 0.99 && screenColor.a > 0.5) {
                        color.rgb *= baseColor.a;
                        color.rgb += (1. - baseColor.a) * screenColor.rgb;
                    }
                }
    ...
    FallBack "Diffuse"
    CustomEditor "SeinEnhancedPBRShaderGUI"
 }
